...
- **FP under `-ffast-math`**: an FP chain (`sin`/`cos`/mul) matches baseline under both precise `-O2` and `-ffast-math` (`fp:5.724743:5.724743`). ConstantFPEncryption preserves FP semantics under reordering.
...
- [ ] **C13** — `-taokari-max`+`-O0`+setjmp. Windows 2026-08-13: `verify_setjmp_unwind_safety.py` is 6/6 including `setjmp max -O0` and `max+vmp -O0` (`jb:54`). Root cause was page-table decrypt in `setjmp` callers (IndGV still walked a table whose fake slots were NULL) plus IRBuilder inserting flattening IR after the entry terminator at `-O0`. Fix: skip EH/NLJ in CFG and stateful decrypt, fill page-table fakes with `taokari.pt.sink`, insert flattening IR before the entry terminator. Gate `setjmp_eh_unwind_safety` is a hard fail. **Not ticked:** Linux not re-run here (`wsl.exe` refused: legacy console). Re-run the same verifier on the WSL clang before ticking.
- [ ] **C21+** — Page-table CIE (`pageTablePoolRef` / cie L4), IndBr fake recovery destinations (L3+), IndGV L2 two-share tables. Windows verifiers `verify_cie_page_table_ref.py`, `verify_indbr_fake_recovery.py`, `verify_indgv_two_share.py` pass; `exceptions_heavy` differential O0/O2 matches; flattening_stress + mba_basic still pass. **Not ticked:** Linux 300/300 differential + full gate suite not re-run here. Re-sync with `scripts/sync-to-wsl.sh` (source path is now derived from the script location) and rebuild `~/taokari-build` before ticking.
