...
- [ ] **C13 (deferred, root-caused)** — The `-taokari-max`+`-O0`+setjmp residual. gdb-traced: SIGSEGV at a page-table byte-load `mov (%rsi,%rdi,1),%dil` inside an internal CIE/CFE constant-decrypt shard called from `main` after `longjmp` returns. The constant-decrypt page-table base pointer (`rsi`) is stale/wrong after longjmp restores the stack — the page-table base lives in stack-local state that `setjmp` did not preserve (or that the restored frame invalidates). Persists even with `deep` (longjmp caller) fully `noobf`-excluded, confirming it is module-level constant-decryption state, not a CFG transform. Proper fix requires the page-table base to be a stable global or recomputed across the setjmp boundary — a substantial change to the constant-encryption infrastructure. Narrow opt-in (`-taokari-max` + setjmp + `-O0`) only; tracked via the skippable `setjmp_eh_unwind_safety` gate. The flattening-specific half is fixed and covered by `verify_setjmp_flatten_safety.py`.
