...
OPMAP_RE = re.compile(
    r"@__taokari_vmp_opmap_(\w+) = private unnamed_addr constant "
    r"\[(\d+) x i64\] \[(.*?)\], align 8",
    re.S,
)
CALL_RE = re.compile(
...
def opcode_encode_map(text: str, name: str) -> dict[int, int]:
    for match in OPMAP_RE.finditer(text):
        if match.group(1) != name:
            continue
        decode = [int(v) for v in I64_RE.findall(match.group(3))]
        return {op: token for token, op in enumerate(decode) if op >= 0}
    raise ValueError(f"missing opcode map for {name}")


def encode(words: list[int], starts: list[int], key: int,
           opcode_encode: dict[int, int], total: int) -> tuple[list[int], list[int]]:
...
def mutate_ir(text: str, name: str, count: int, key: int,
              opcode_encode: dict[int, int],
              words: list[int], starts: list[int]) -> str:
    count = max(count, len(words))
    encoded, pcmap = encode(words, starts, key, opcode_encode, count)
